...
export async function getPortalToken(tenant) {
  ...
  // Always use 'meydan' as the domain parameter regardless of tenant
  const domain = 'meydan';
  const candidates = [
    // OAuth login path used by the portal UI (works on modern installs)
    { url: `${base}/api/v2/config/login/oauth`, body: { domain, username: process.env.API_USERNAME, password: process.env.API_PASSWORD } },
    // v2 login using domain (fallback for older back-ends)
    { url: `${base}/api/v2/login`, body: { domain, username: process.env.API_USERNAME, password: process.env.API_PASSWORD } },
    // very old legacy login path
    { url: `${base}/api/login`, body: { domain, username: process.env.API_USERNAME, password: process.env.API_PASSWORD } },
  ];
