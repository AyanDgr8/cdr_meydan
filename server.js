...
app.post('/api/reports/progressive/init', async (req, res) => {
  ...
  try {
    ...
    // Format the start and end dates with time components for proper comparison
    ...
    const startDateTime = formatDateTimeForDB(startDateObj);
    const endDateTime = formatDateTimeForDB(endDateObj);

    console.log(`Filtering records between: ${startDateTime} and ${endDateTime}`);


    // Use proper date range filtering that works across months
    // Generate individual date conditions for each day in the range
